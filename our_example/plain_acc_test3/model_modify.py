...
def set_everything_seed(seed: int):
    random.seed(seed)
    np.random.seed(seed)
    ...
    # 保证 CUDA 算法的确定性 (会稍微降低性能，但保证结果一致)
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    
    # 使用 Hugging Face 官方的 set_seed (涵盖了模型初始化等)
    set_seed(seed)
    
    # 强制设置 PyTorch 确定性算法 (可选，极严格模式)
...
def main():
    ...
    parser.add_argument("--gradient_accumulation_steps", type=int, default=1,
                        help="梯度累积，适配小显存GPU")
    parser.add_argument("--disable_tqdm", action="store_true", help="Disable tqdm progress bar for cleaner logs")
    parser.add_argument("--seed", type=int, default=169, help="Random seed for reproducibility")
    args = parser.parse_args()


    set_everything_seed(args.seed)
