...
def inv_sqrt(self, k1=1, k2=12, L=2048.0):
    ...
    if method == "ideal":
        ...
        # 2. 正确解包 (stack 后，直接按索引取)
        eps_cmp = plain_block_2[0]           # Shape: (Size)
        delta_cmp = plain_block_2[1]         # Shape: (Size)
        masked_sign_plain = plain_block_2[2] # Shape: (Size)
...
def gelu(self, approximate="none", k1=1, k2=12):
    ...
    if method == "ideal":
        ...
        comm_block_1 = crypten.cat([
            eps_cmp_share,               # 索引 0, 1
            delta_cmp_share,             # 索引 2, 3
            delta_erf_share.unsqueeze(0) # 索引 4
        ], dim=0)
        ...
        # 2. 精确解包 (避免使用切片带来的歧义)
        eps_cmp = plain_block_1[0:2]      # Shape: (2, Size)
        delta_cmp = plain_block_1[2:4]    # Shape: (2, Size)
        delta_erf_plain_1 = plain_block_1[4] # Shape: (Size)
