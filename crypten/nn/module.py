...
# Modified by Andes Y. L. Kei: Implemented Embedding, GELU, LayerNormalization, and other modules required by transformers
#
...
class Module:
    ...
    def register_parameter(self, name, param, requires_grad=True):
        ...
        if param.dtype not in [torch.float32, torch.float64, torch.complex64, torch.complex128, torch.float16, torch.bfloat16]:
            print("Warning: crypten.nn.Module.register_parameter() attempts to set require_grad=True for tensor of int or bool dtype")
            requires_grad = False
...
class Graph(Container):
    ...
    def __init__(self, input_names, output_names, modules=None, graph=None):
        ...
        self.softmax_time = 0
        self.gelu_time = 0
        self.layernorm_time = 0
        self.other_time = 0

        self.total_comm_bytes = 0
        self.total_comm_time = 0
        self.embedding_comm_bytes = 0
        self.embedding_comm_time = 0
        self.matmul_comm_bytes = 0
        self.matmul_comm_time = 0
        self.softmax_comm_bytes = 0
        self.softmax_comm_time = 0
        self.gelu_comm_bytes = 0
        self.gelu_comm_time = 0
        self.layernorm_comm_bytes = 0
        self.layernorm_comm_time = 0
        self.other_comm_bytes = 0
        self.other_comm_time = 0
    ...
    def forward(self, *args):
        ...
        self.embedding_comm_bytes = 0
        self.embedding_comm_time = 0
        self.embedding_comm_rounds = 0
        self.matmul_comm_bytes = 0
        self.matmul_comm_time = 0
        self.matmul_comm_rounds = 0
        self.softmax_comm_bytes = 0
        self.softmax_comm_time = 0
        self.softmax_comm_rounds = 0
        self.gelu_comm_bytes = 0
        self.gelu_comm_time = 0
        self.gelu_comm_rounds = 0
        self.layernorm_comm_bytes = 0
        self.layernorm_comm_time = 0
        self.layernorm_comm_rounds = 0
        self.tanh_comm_bytes = 0
        self.tanh_comm_time = 0
        self.tanh_comm_rounds = 0
        self.conv_comm_bytes = 0
        self.conv_comm_time = 0
        self.conv_comm_rounds = 0
        self.other_comm_bytes = 0
        self.other_comm_time = 0
        self.other_comm_rounds = 0
        ...
        while node_to_compute is not None:
            ...
            if (isinstance(module, Embedding)):
                ...
                self.embedding_comm_bytes += comm_bytes
                self.embedding_comm_time += comm_time
                self.embedding_comm_rounds += comm_rounds
            elif (isinstance(module, (MatMul, Gemm))):
                self.matmul_time += module_time
                self.matmul_comm_bytes += comm_bytes
                self.matmul_comm_time += comm_time
                self.matmul_comm_rounds += comm_rounds
            elif (isinstance(module, Softmax)):
                self.softmax_time += module_time
                self.softmax_comm_bytes += comm_bytes
                self.softmax_comm_time += comm_time
                self.softmax_comm_rounds += comm_rounds
            elif (isinstance(module, GELU)):
                self.gelu_time += module_time
                self.gelu_comm_bytes += comm_bytes
                self.gelu_comm_time += comm_time
                self.gelu_comm_rounds += comm_rounds
            elif isinstance(module, LayerNormalization):
                self.layernorm_time += module_time
                self.layernorm_comm_bytes += comm_bytes
                self.layernorm_comm_time += comm_time
                self.layernorm_comm_rounds += comm_rounds
            elif isinstance(module, Tanh):
                self.tanh_time += module_time
                self.tanh_comm_bytes += comm_bytes
                self.tanh_comm_time += comm_time
                self.tanh_comm_rounds += comm_rounds
            elif isinstance(module, (Conv, Conv1d, Conv2d)):
                self.conv_time += module_time
                self.conv_comm_bytes += comm_bytes
                self.conv_comm_time += comm_time
                self.conv_comm_rounds += comm_rounds
            else:
                self.other_time = module_time
                self.other_comm_bytes += comm_bytes
                self.other_comm_time += comm_time
                self.other_comm_rounds += comm_rounds
            self.total_comm_bytes += comm_bytes
            self.total_comm_time += comm_time
            self.total_comm_rounds += comm_rounds
            ...
            if all(computed[output_name] for output_name in self.output_names):
                ...
                report_cost = cfg.debug.report_cost
                if report_cost:
                    print("====================================================")
                    print(f"total_running_time: {time.time() - forward_start_time} sec")
                    print(f"embedding_time: {self.embedding_time} sec")
                    print(f"matmul_time: {self.matmul_time} sec")
                    print(f"softmax_time: {self.softmax_time} sec")
                    print(f"gelu_time: {self.gelu_time} sec")
                    print(f"layernorm_time: {self.layernorm_time} sec")
                    print(f"tanh_time: {self.tanh_time} sec")
                    print(f"conv_time: {self.conv_time} sec")
                    print(f"other_time: {self.other_time} sec")
                    print("====================================================")
                    print(f"total_comm_time: {self.total_comm_time} sec")
                    print(f"embedding_comm_time: {self.embedding_comm_time} sec")
                    print(f"matmul_comm_time: {self.matmul_comm_time} sec")
                    print(f"softmax_comm_time: {self.softmax_comm_time} sec")
                    print(f"gelu_comm_time: {self.gelu_comm_time} sec")
                    print(f"layernorm_comm_time: {self.layernorm_comm_time} sec")
                    print(f"tanh_comm_time: {self.tanh_comm_time} sec")
                    print(f"conv_comm_time: {self.conv_comm_time} sec")
                    print(f"other_comm_time: {self.other_comm_time} sec")
                    print("====================================================")
                    print(f"total_comm_bytes: {self.total_comm_bytes/ 1073741824} GB")
                    print(f"embedding_comm_bytes: {self.embedding_comm_bytes / 1073741824} GB")
                    print(f"matmul_comm_bytes: {self.matmul_comm_bytes / 1073741824} GB")
                    print(f"softmax_comm_bytes: {self.softmax_comm_bytes / 1073741824} GB")
                    print(f"gelu_comm_bytes: {self.gelu_comm_bytes / 1073741824} GB")
                    print(f"layernorm_comm_bytes: {self.layernorm_comm_bytes / 1073741824} GB")
                    print(f"tanh_comm_bytes: {self.tanh_comm_bytes / 1073741824} GB")
                    print(f"conv_comm_bytes: {self.conv_comm_bytes / 1073741824} GB")
                    print(f"other_comm_bytes: {self.other_comm_bytes / 1073741824} GB")
                    print("====================================================")
                    print(f"total_comm_rounds: {self.total_comm_rounds}")
                    print(f"embedding_comm_rounds: {self.embedding_comm_rounds}")
                    print(f"matmul_comm_rounds: {self.matmul_comm_rounds}")
                    print(f"softmax_comm_rounds: {self.softmax_comm_rounds}")
                    print(f"gelu_comm_rounds: {self.gelu_comm_rounds}")
                    print(f"layernorm_comm_rounds: {self.layernorm_comm_rounds}")
                    print(f"tanh_comm_rounds: {self.tanh_comm_rounds}")
                    print(f"conv_comm_rounds: {self.conv_comm_rounds}")
                    print(f"other_comm_rounds: {self.other_comm_rounds}")
                    print("====================================================")
                
                estimate_cost, estimate_mode = cfg.cost.estimate_cost, cfg.cost.estimate_mode
                if crypten.comm.get().get_rank() == 0 and estimate_cost:
                    total_time = time.time() - forward_start_time
                    if estimate_mode == "comp":
                        print(f"comp time: {total_time:.2f}s")
                    elif estimate_mode == "comm":
                        print(f"comm byte: {self.total_comm_bytes / 1073741824:.2f} GB, round: {self.total_comm_rounds}")
                    else:
                        print(f"running time: {total_time:.2f}s, comm byte: {self.total_comm_bytes / 1073741824:.2f} GB")
            ...
            # this can save memory but will significantly increase running time
...
class Constant(Module):
    ...
    def __init__(self, value):
        ...
        self.register_buffer("value", value.to(dtype=torch.float))
...
class Slice(Module):
    ...
    def forward(self, x):
        ...
        for idx, axis in enumerate(axes):
            ...
            in_size = output.size(int(axis))
            
            start = in_size + start if start < 0 else start
            if end < -in_size:
                end = -end
            elif end < 0:
                end = in_size + end
            length = min(end, in_size) - start
...
class Shape(Module):
    ...
    def forward(self, x, dim=None):
        ...
        if dim is None:
            size = torch.tensor(x.size(), device=x.device)
        else:
            size = torch.tensor(x.size(dim), device=x.device)
        return size
...
class Gather(Module):
    ...
    def forward(self, input):
        ...
        if indices.is_cuda: tensor = tensor.to("cuda")
    ...
    def from_onnx(attributes=None):
        if attributes is None:
            attributes = {}
        return Gather(
            dimension=attributes.get("axis", 0),
        )
...
class LayerNormalization(Module):
    ...
    def __init__(self, axis=-1, eps=1e-05):
        super().__init__()
        self.axis = axis
        self.eps = eps
...
class Identity(Module):
    """
    Module that performs identity operation following the ONNX specification.
    """
    def __init__(self):
        super().__init__()
...
class GELU(Module):
    r"""
    Module that computes Gaussian Error Linear Unit (GELU) activations element-wise.

    :math:`\text{GELU}(x) = 0.5x (1 + \text{erf}(x / \sqrt{2}))`
    """
    def __init__(self, approximate="none", inplace=False):
        super().__init__()
        self.approximate = str(approximate, encoding='utf-8')
        if inplace:
...
class Embedding(Module):
    ...
    """
    # TODO: this implementation may not support private training
    def __init__(self):
        super().__init__()
    ...
    def forward(self, input):
        ...
        if crypten.is_encrypted_tensor(indices):
            # obtain one-hot pair from dealer
            provider = crypten.mpc.get_default_provider()
            r, v = provider.generate_one_hot_pair(indices.size(), voc_size, device=indices.device)

            # reconstruct masked index
            j = indices - r
            j = j.get_plain_text().int().unsqueeze(-1) % voc_size

            # obtain the correct one-hot vector by shifting
            arange1 = torch.arange(voc_size, device=indices.device).view([1,1,voc_size]).repeat((indices.shape[0],indices.shape[1],1))
            arange2 = (arange1 - j) % voc_size
            indices = v.gather(2, arange2)

            # "table lookup" via matmul
